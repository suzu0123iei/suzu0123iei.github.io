const I18N = {
      ja: {
        ...
        "hero.title":'<span class="ttl-line">mk48.io を <span class="accent nowrap">“楽しもう”</span></span><span class="ttl-line">初心者向け非公式サイト</span>',
        "hero.lead":"",
      ...
      en: {
        ...
        "hero.kicker":"Unofficial guide for beginners",
        "hero.title":'<span class="ttl-line">Enjoy <span class="accent nowrap">mk48.io</span></span><span class="ttl-line">A fan-made beginner site</span>',
        "hero.lead":"",
...
,
        ...
        "yt.p":"Unofficially 소개—here’s a channel many players find helpful.",
        "yt.name":"● Seiran (Mr./Ms. Seiran)",
      ...
      zh: {
        ...
        "btn.play":"立即开始",
        "btn.ships":"查看点数&舰艇",
        "btn.youtuber":"查看YouTuber",
        "btn.reset":"重置",
        "drawer.title":"菜单",

        "hero.kicker":"新手向非官方指南",
        "hero.title":'<span class="ttl-line">一起 <span class="accent nowrap">玩 mk48.io</span></span><span class="ttl-line">新手向非官方站点</span>',
        "hero.lead":"",
...
function applyI18n(){
    ...
    function setLang(lang){
      ...
      // update buttons
      $$(".langBtn").forEach(b => b.setAttribute("aria-pressed", b.dataset.lang === lang ? "true" : "false"));
...
// Language buttons
      ...
      // Opening animation (OP): fade out a bit later for a calmer feel
      const opening = document.getElementById("opening-layer");
      const header = document.getElementById("mainHeader");
      if (opening){
        // Start radar ping waves
        opening.querySelectorAll(".op-ping").forEach((el, i)=>{
          window.setTimeout(()=> el.classList.add("active"), 120 + i*220);
        });

        // Show header once the first beat is visible
        if (header) window.setTimeout(()=> header.classList.remove("hidden-start"), 350);

        // Keep OP slightly longer, then fade out
        const HIDE_DELAY_MS = 2600;
        const REMOVE_AFTER_MS = 5200;
        window.setTimeout(()=> opening.classList.add("hidden"), HIDE_DELAY_MS);
        window.setTimeout(()=> opening.remove(), REMOVE_AFTER_MS);
      } else {
        if (header) header.classList.remove("hidden-start");
      }
    }
